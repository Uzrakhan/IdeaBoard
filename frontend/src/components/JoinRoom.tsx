...
const JoinRoom: React.FC<JoinRoomProps> = ({ room }) => {
    ...
    // ✅ Safe check for approved member
    const isApprovedMember = room.members.some((m) => {
        const memberUserId =
            typeof m.user === 'object' && '_id' in m.user
                ? (m.user as any)._id
                : m.user;

        return memberUserId?.toString() === userId && m.status === 'approved';
    });

    const isOwner =
        typeof room.owner === 'object' && '_id' in room.owner
            ? (room.owner as any)._id?.toString() === userId
            : typeof room.owner === 'string' || typeof room.owner === 'number'
                ? (room.owner as string | number).toString() === userId
                : false;

    const hasPendingRequest = room.members.some((m) => {
        const memberUserId =
            typeof m.user === 'object' && '_id' in m.user
                ? (m.user as any)._id
                : m.user;

        return memberUserId?.toString() === userId && m.status === 'pending';
    });
                ...
                <div className="flex items-center bg-gray-50 rounded-lg p-6 mb-8">
                    ...
                    <div className="ml-4">
                        <h3 className="font-medium text-gray-800 text-lg">
                            {(room.owner as any)?.username || 'Owner'}
                        </h3>
                ...
                <div className="flex items-center justify-center text-gray-600 text-base mb-6">
                    <svg className="w-5 h-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path
                            ...
                            strokeLinejoin="round"
                            strokeWidth="2"
                            d="M17 20h5v-2a3 3 0 00-3-3H5a3 3 0 00-3 3v2h5m0 0a3 3 0 003 3h4a3 3 0 003-3m-7.5-2.5a3 3 0 11-6 0 3 3 0 016 0zm3.5 0a3 3 0 11-6 0 3 3 0 016 0z"
                        />
                    </svg>
                    <span>
                        {
