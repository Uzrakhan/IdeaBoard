...
const JoinRoom: React.FC<JoinRoomProps> = ({  room }) => {
    ...
    //removed the internal useEffect for fethcing room details here
    //as the 'room' prop is now passed from JoinRoomWrapper
    /*
    useEffect(() => {
    ...
    */
    ...
    const handleJoinRequest = async () => {
        ...
        try {
            ...
            // Note: Actual room status update (from pending to approved) would typically
            // come from a WebSocket message or a periodic refetch initiated by the owner's approval.
            // For this component, simply updating the message is sufficient.
        } catch (err: any) {
    ...
    useEffect(() => {
        if(isApprovedMember && !isOwner) { // Only redirect if approved and not the owner
