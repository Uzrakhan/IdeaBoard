...
io.on('connection', (socket: Socket) => { // 'socket' here will infer type correctly from 'socket.io'
  ...
  ///4. Approve member
  socket.on("room:approveMember", async ({ roomCode, memberId }) => {

    await Room.updateOne(
      { roomCode, "members.user": memberId },
    ...
    const userSocketId = connectedUsers.get(memberId);

    if (userSocketId) {
      io.to(userSocketId).emit("yourRoomStatusUpdated", {
        status: "approved",
        roomCode
      });
    }
...
/// 5. reject member
  socket.on("room:rejectMember", async ({ roomCode, memberId }) => {
    console.log("🔥 REJECT EVENT RECEIVED");
    console.log("Room:", roomCode, "Member:", memberId);
    ...
    io.to(memberId).emit("yourRoomStatusUpdated", {
      status: "rejected",
      roomCode
    });
...
io.on('connection', (socket: Socket) => { // 'socket' here will infer type correctly from 'socket.io'
  ...
  socket.on("whiteboard:pan", ({ roomCode, pan }) => {
    socket.to(roomCode).emit("whiteboard:stateUpdate", { pan });
  });

  socket.on("whiteboard:zoom", ({ roomCode, zoom }) => {
    socket.to(roomCode).emit("whiteboard:stateUpdate", { zoom });
  });
